...
async def config_cmd(message: Message, session: AsyncSession):
    ...
    await message.answer(f"1. /quiz_restart - пройти квиз-опрос заново"
                         ...
                         f"\n\n4. /service - встать на плановое ТО"
                         f"\n\n〰️〰️〰️〰️〰️〰️〰️〰️〰️\n"
...
async def promo_cmd(message: Message, session: AsyncSession):
    ...
    share_text = (f"🔍 Ищете коляску?"
                  f"\nПодберем надежную модель под ваши условия"
                  f"\n\n🛠 Уже купили?"
                  f"\nУзнайте, как случайно не сломать её (80% поломок — вина владельцев!)"
                  f"\n\n{promo_code} - промокод для бесплатной активации (скопируйте его)")
